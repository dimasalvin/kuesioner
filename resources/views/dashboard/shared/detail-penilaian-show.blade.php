...
@php
$user = auth()->user();
if ($user->isAdmin())       $backRoute = route('dashboard.admin.detail-penilaian-list',       ['id'=>$row->nakes_id,'tipe'=>$tipe]);
elseif ($user->isManagement()) $backRoute = route('dashboard.management.detail-penilaian-list', ['id'=>$row->nakes_id,'tipe'=>$tipe]);
else                        $backRoute = route('dashboard.user.detail-penilaian');

$totalSkor = 0;
// Hitung total dari slot q1-q15 yang ada nilainya (>0)
for ($i=1;$i<=15;$i++) {
    $val = $row->{"q{$i}"} ?? 0;
    if ($val > 0) $totalSkor += $val;
}
...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
    ...
    <div style="font-size:14px;color:var(--muted);">
        Detail penilaian dari <strong>{{ $row->pasien_nama }}</strong>
        untuk <strong>{{ $row->nakes_nama }}</strong>
    </div>
</div>

{{-- Summary card --}}
<div style="background:linear-gradient(135deg,var(--teal),var(--teal-dark));border-radius:var(--radius);
            padding:20px 24px;margin-bottom:20px;color:white;display:flex;
    ...
    <div>
        <div style="font-size:12px;opacity:.8;margin-bottom:4px;">
            {{ $tipe==='dokter' ? '👨‍⚕️' : '👩‍⚕️' }} {{ ucfirst($tipe) }}
        </div>
        <div style="font-family:'Caveat',cursive;font-size:24px;line-height:1.2;">
            {{ $row->nakes_nama }}
        </div>
        @if(isset($row->spesialisasi) && $row->spesialisasi)
        <div style="font-size:12px;opacity:.75;margin-top:2px;">{{ $row->spesialisasi }}</div>
        @endif
        <div style="font-size:12px;opacity:.7;margin-top:8px;">
            Pasien: {{ $row->pasien_nama }} • {{ \Carbon\Carbon::parse($row->created_at)->indonesiaFormat() }}
        </div>
    </div>
